...
export default function Testimonials() {
    ...
    return (
        <section id="testimonios" className="py-24 bg-[#080808] relative overflow-hidden">
            ...
            <div className="absolute top-1/2 right-1/4 translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-[#39FF14]/5 rounded-full blur-[100px] pointer-events-none"></div>
            ...
            <div className="container mx-auto px-6 lg:px-12 max-w-7xl relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
                    {testimonials.map((testimonial) => (
                        <div key={testimonial.id} className="bg-[#141414] border border-[#1C1C1E] hover:border-[#0070F3]/30 rounded-2xl p-8 transition-all duration-300 shadow-xl relative group">
                            ...
                            <div className="flex gap-1 mb-6">
                                {[...Array(5)].map((_, i) => (
                                    <Star key={i} size={16} className="fill-[#39FF14] text-[#39FF14]" />
                                ))}
                            </div>
                ...
                <div className="max-w-2xl mx-auto bg-[#141414] border border-[#1C1C1E] rounded-2xl p-8 mb-16 shadow-2xl relative">
                    <h3 className="text-xl font-bold text-white mb-6 text-center">Cuéntanos tu experiencia con PSMILE</h3>

                    {submitted ? (
                        <div className="text-center py-8 animate-fade-in">
                            <div className="inline-flex justify-center items-center w-16 h-16 bg-[#39FF14]/10 rounded-full mb-4 border border-[#39FF14]/30">
                                <CheckCircle2 size={32} className="text-[#39FF14]" />
                            </div>
                            <h4 className="text-lg font-bold text-white mb-2">¡Gracias por tu testimonio!</h4>
                            <p className="text-[#9CA3AF] text-sm">Tu experiencia ayuda a otros jugadores a alcanzar su máximo potencial.</p>
                        </div>
                    ) : (
                        <form onSubmit={handleSubmit} className="space-y-6">
                            {/* Interactive Stars */}
                            <div className="flex flex-col items-center gap-2 mb-2">
                                <span className="text-[10px] font-bold text-[#A1A1AA] tracking-widest uppercase">Calificación</span>
                                <div className="flex gap-2">
                                    {[1, 2, 3, 4, 5].map((star) => (
                                        ...
                                        >
                                            <Star
                                                size={28}
                                                className={`transition-colors duration-200 ${(hoverRating || rating) >= star ? 'fill-[#39FF14] text-[#39FF14]' : 'text-[#333]'}`}
                                            />
                                        </button>
                                    ))}
                                </div>
                            ...
                            <div className="grid grid-cols-1 gap-6">
                                <div>
                                    <label className="block text-[10px] font-bold text-[#A1A1AA] tracking-widest uppercase mb-2">Nombre y Apellido (o Iniciales)</label>
                                    <input
                                        type="text"
                                        required
                                        value={formData.name}
                                        onChange={e => setFormData({ ...formData, name: e.target.value })}
                                        className="w-full bg-[#1A1A1A] border border-white/10 rounded-lg px-4 py-3 text-white placeholder-white/30 outline-none focus:border-[#0070F3] transition-colors text-sm"
                                        placeholder="Ej. Martín V. o Familia Díaz"
                                    />
                                </div>

                                <div>
                                    <label className="block text-[10px] font-bold text-[#A1A1AA] tracking-widest uppercase mb-2">Tu Comentario</label>
                                    <textarea
                                        required
                                        rows="3"
                                        value={formData.comment}
                                        onChange={e => setFormData({ ...formData, comment: e.target.value })}
                                        className="w-full bg-[#1A1A1A] border border-white/10 rounded-lg px-4 py-3 text-white placeholder-white/30 outline-none focus:border-[#0070F3] transition-colors text-sm resize-none"
                            ...
                            <button
                                ...
                                className="w-full bg-[#0070F3] hover:bg-[#0056B3] text-white font-black px-8 py-3 rounded-lg text-sm transition-all uppercase tracking-wider disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                Enviar Testimonio
                            </button>
                        </form>
                ...
                <div className="flex flex-col items-center justify-center">
                    ...
                    <div className="inline-block bg-[#1A1A1A] border border-white/5 rounded-lg px-6 py-3">
                        <p className="text-[10px] md:text-xs font-bold text-[#9CA3AF] text-center tracking-widest uppercase">
                            Metodología basada en instrumentos psicométricos estructurados (EPI, GRID, ePsD Lite) y Terapia Breve Centrada en Soluciones.
                        </p>
                    </div>
                </div>
