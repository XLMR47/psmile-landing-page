...
export default function FacilitadorPanel() {
  ...
  const handleAvanzar = () => avanzarBloque(sesionId, sesion.bloqueActual, totalBloques);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ background: '#131d30', border: '1px solid #1a2640', borderRadius: 10, overflow: 'hidden', textAlign: 'center', padding: 8 }}>
        ...
        <canvas ref={canvasRef} width={180} height={280} style={{ borderRadius: 8, maxWidth: '100%' }} />
      </div>
    </div>
  );
}
...
function drawSilhouetteMini(ctx, w, h) {
  ...
  ctx.beginPath(); ctx.moveTo(78*sx,82*sy); ctx.lineTo(162*sx,82*sy); ctx.lineTo(155*sx,200*sy); ctx.lineTo(85*sx,200*sy); ctx.closePath(); ctx.fill(); ctx.stroke();
  ctx.restore();
}
