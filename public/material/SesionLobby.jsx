...
export function LobbyJugador() {
  ...
  useEffect(() => {
    if (!sesionId) return;
    const { doc: fbDoc, onSnapshot } = require('firebase/firestore');
    const unsub = onSnapshot(fbDoc(db, 'sesiones', sesionId), (snap) => {
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="mb-4">
            ...
            <input
              ...
              maxLength={10}
              className="w-full bg-[#111827] border border-white/10 focus:border-[#0070F3] rounded-xl px-4 py-3.5 text-white text-center text-2xl font-black tracking-widest uppercase outline-none transition-colors placeholder-[#374151]"
              style={{ fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: '6px' }}
            />
          </div>
          ...
          <div className="mb-6">
            ...
            <input
              ...
              maxLength={30}
              className="w-full bg-[#111827] border border-white/10 focus:border-[#0070F3] rounded-xl px-4 py-3.5 text-white text-sm outline-none transition-colors placeholder-[#4B5563]"
            />
          </div>
          ...
          <button
            ...
            className="w-full bg-[#0070F3] hover:bg-[#0060D0] disabled:opacity-40 text-white font-black text-sm uppercase tracking-widest py-4 rounded-xl transition-all flex items-center justify-center gap-2"
          >
            {entrando
